...
- If `_Agent-Context/WIKI-CHANGES.md` (written nightly by `tools/nightly_compile.py` via `tools/wiki_changes.py`, LLM-free) is under 36 hours old, add a short "wiki last night" section: its Changed bullets and Linked line verbatim, then every Flagged bullet. A new Contested or Superseded bullet, or a Conflict line (a new source that disagrees with an existing page, from `tools/contradiction_check.py`, kept 30 days in `_Agent-Context/CONTRADICTIONS.md`), is a decision for the owner and goes into the quick actions with both pages linked. If it is 36 hours or older, say in the health block that the compile brief is stale. Copy its "waiting for your decision" lines (concept proposals, answered in Buzz #dreaming) as well; one waiting 2 days or more is a red flag like an overdue promise.
