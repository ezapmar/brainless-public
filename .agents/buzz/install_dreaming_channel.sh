#!/usr/bin/env bash
# Create the Buzz identity + channel for dreaming (tools/dreaming.py): nightly
# link and merge proposals the owner answers with yes / no / skip.
#   bash .agents/buzz/install_dreaming_channel.sh
# Generates the "dreaming" key (once), registers it as a relay member, creates
# the #dreaming channel with the moderator identity if missing, caches its uuid
# in channels.json, and adds the owner + dreaming as members. Idempotent: an
...
CHANNEL_NAME="${DREAMING_CHANNEL:-dreaming}"
IDENTITY="dreaming"
...
log() { echo "[install_dreaming_channel] $*" >&2; }
...
if [ -z "$CID" ]; then
  BUZZ_PRIVATE_KEY="$(secret_of moderator)" buzz channels create --name "$CHANNEL_NAME" --type stream --visibility open \
    --description "Nightly link and merge proposals from the wiki; reply yes / no / skip" >/dev/null
  CID="$(channel_id)"
...
log "done: #$CHANNEL_NAME ready, dreaming is a member"
