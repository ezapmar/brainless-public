#!/usr/bin/env bash
# Buzz Katman 1: post today's thinking surface (THINKING.md) to #thinking once,
# as the "dusunce" identity. Runs as ExecStartPost of brainless-think-surface,
...
if printf '%s\n' "$BODY" | "$VAULT/.agents/scripts/buzz_post.sh" dusunce thinking 2>&1 | grep -q "posted as"; then
