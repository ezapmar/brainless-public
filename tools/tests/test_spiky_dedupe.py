...
class TestDuplicateDetection(unittest.TestCase):
    def test_same_task_with_different_turkish_suffixes(self):
        self.assertTrue(spiky.is_duplicate(
            "Ahmet ile bordro entegrasyonunu konus",
            ["Ahmet ile bordro entegrasyonu konusulacak"]))
    ...
    def test_two_different_tasks_about_the_same_project_are_kept(self):
        # The expensive false positive: both mention Garanti, both name a
        # person, and dropping one loses real work.
        self.assertFalse(spiky.is_duplicate(
            "Garanti API dokumanini Ege'ye ilet",
            ["Garanti dashboard'unu Sinem'e goster"]))

    def test_unrelated_task_is_kept(self):
        self.assertFalse(spiky.is_duplicate(
            "Sinem'e dashboard gonder", ["Ege ile vergi konusunu netlestir"]))

    def test_empty_ledger_keeps_everything(self):
        self.assertFalse(spiky.is_duplicate("QNB teklifini hazirla", []))
